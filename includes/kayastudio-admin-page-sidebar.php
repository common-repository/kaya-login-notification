<?php

/**
 * Kaya Studio - Admin Main Page Sidebar
 * Displays Kaya Studio admin main page sidebar.
 *
 * @version 1.0.0
 */

/**
 * Displays Kaya Studio page sidebar.
 */
if (!function_exists('kayastudio_plugins_admin_doMainPageSidebar'))
{
	function kayastudio_plugins_admin_doMainPageSidebar()
	{
		if (!current_user_can('edit_posts'))
		{
			wp_die('<p>' . __('You do not have sufficient permissions.') . '</p>');
		}

?>

		<div class="ks-wp-dashboard-page-card">
			<div class="ks-wp-dashboard-page-card-header">
				<?php echo esc_html__('Donate', WPKLN_TEXT_DOMAIN); ?>
			</div>
			<div class="ks-wp-dashboard-page-card-body">
				<h5 class="ks-wp-dashboard-page-card-title"><?php echo esc_html__('Support the advancement of this plugin', WPKLN_TEXT_DOMAIN); ?>&nbsp;&#9829;</h5>
				<p class="ks-wp-dashboard-page-card-text">
					<?php echo esc_html__('You can give to support this plugin development and to encourage the developer for making open source softwares.', WPKLN_TEXT_DOMAIN); ?>
				</p>
				<p class="ks-wp-dashboard-page-card-text">
					<?php echo esc_html__('Make a donation with Paypal:', WPKLN_TEXT_DOMAIN); ?>
				</p>
				<p class="ks-wp-dashboard-page-card-text">
					<a href="https://www.paypal.me/dotkaya" class="ks-wp-dashboard-page-btn ks-wp-dashboard-page-btn-primary" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Donate', WPKLN_TEXT_DOMAIN); ?></a>
				</p>
				<p class="ks-wp-dashboard-page-card-text">
					<?php echo esc_html__('Make a donation with Bitcoin:', WPKLN_TEXT_DOMAIN); ?></p>
				<p class="ks-wp-dashboard-page-card-text">
					<a href="bitcoin:1DosbNiE4L8HPBJrwYpWmcomTyh3Yxy5oU?label=KayaStudio" target="_blank" rel="noopener noreferrer">
						<img src="data:image/png;base64," alt="Make a donation with Bitcoin" class="ks-wp-dashboard-page-qrcode" style="width: auto; height: auto; max-width: 100%;">
					</a>
				</p>
			</div>
		</div>

<?php
	}
}
